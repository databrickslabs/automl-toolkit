...
class ManualRunner(dataPayload: DataGeneration) extends AutomationRunner(dataPayload.data) {


  override def exploreFeatureImportances(): FeatureImportanceReturn = {
    val featureResults = new RandomForestFeatureImportance(dataPayload.data, _featureImportancesConfig,
    ...
    params.FeatureImportanceReturn(featureResults._1, featureResults._2, featureResults._3, dataPayload.modelType)
  }
  ...
  override def generateDecisionSplits(): TreeSplitReport = {
    ...
    new DecisionTreeSplits(dataPayload.data, _treeSplitsConfig, dataPayload.modelType)
  ...
  override def runFeatureCullingWithPrediction(): FeatureImportancePredictionOutput = {

    val featureImportanceResults = exploreFeatureImportances()
    val selectableFields = featureImportanceResults.fields :+ _mainConfig.labelCol

    val dataSubset = dataPayload.data.select(selectableFields.map(col):_*)
    val payload = DataGeneration(dataSubset, selectableFields, dataPayload.modelType)
