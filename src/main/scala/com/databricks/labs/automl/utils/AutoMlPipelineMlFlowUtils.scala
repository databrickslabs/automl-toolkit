...
object AutoMlPipelineMlFlowUtils {
  ...
  def logTagsToMlFlow(pipelineId: String, tags: Map[String, String]): Unit = {
    val mlFlowRunIdAndConfig = AutoMlPipelineMlFlowUtils.getMainConfigByPipelineId(pipelineId)
    if(mlFlowRunIdAndConfig.mainConfig.mlFlowLoggingFlag) {
      val mlflowTracker = MLFlowTracker(mlFlowRunIdAndConfig.mainConfig.mlFlowConfig)
  ...
  def saveInferencePipelineDfAndLogToMlFlow(pipelineId: String,
    ...
    if(mlFlowRunIdAndConfig.mainConfig.mlFlowLoggingFlag) {
      ...
      val pipelineDir = s"$baseDirectory${modelDescriptor}_${mlFlowRunIdAndConfig.mlFlowRunId}/BestPipeline/"
      val finalPipelineSavePath = Paths.get(pipelineDir).toString
      logger.info(s"Saving pipeline id $pipelineId to path $finalPipelineSavePath")
      finalPipelineModel.save(finalPipelineSavePath)
      logger.info(s"Saved pipeline id $pipelineId to path $finalPipelineSavePath")
      logTagsToMlFlow(pipelineId, Map(PipelineMlFlowTagKeys.PIPELINE_MODEL_SAVE_PATH_KEY -> finalPipelineSavePath))
      // Save TrainingDf and log to MlFlow
      val trainDfBaseDirectory = Paths.get(s"$mlFlowModelSaveDirectory/FeatureEngineeredDataset/")
      val trainDfDir = s"$trainDfBaseDirectory${modelDescriptor}_${mlFlowRunIdAndConfig.mlFlowRunId}/data/"
