...
class LogisticRegressionTuner(df: DataFrame)
  ...
  private var _logisticRegressionNumericBoundaries =
    _logisticRegressionDefaultNumBoundaries
  ...
  def setLogisticRegressionNumericBoundaries(
  ...
  def getLogisticRegressionNumericBoundaries: Map[String, (Double, Double)] =
    _logisticRegressionNumericBoundaries
  ...
  private def returnBestHyperParameters(
    collection: ArrayBuffer[LogisticRegressionModelsWithResults]
  ...
  private def sortAndReturnAll(
    results: ArrayBuffer[LogisticRegressionModelsWithResults]
  ...
  private def generateThresholdedParams(
    ...
    var i = 0
    do {
      val elasticNetParams = generateRandomDouble(
        "elasticNetParams",
        _logisticRegressionNumericBoundaries
      )
      val fitIntercept = coinFlip()
      val maxIter =
        generateRandomInteger("maxIter", _logisticRegressionNumericBoundaries)
      val regParam =
        generateRandomDouble("regParam", _logisticRegressionNumericBoundaries)
      val standardization = coinFlip()
      val tolerance =
        generateRandomDouble("tolerance", _logisticRegressionNumericBoundaries)
      iterations += LogisticRegressionConfig(
        elasticNetParams,
        fitIntercept,
        maxIter,
        regParam,
  ...
  private def generateAndScoreLogisticRegression(
    train: DataFrame,
    test: DataFrame,
    modelConfig: LogisticRegressionConfig,
    generation: Int = 1
  ...
  private def runBattery(
    battery: Array[LogisticRegressionConfig],
    generation: Int = 1
  ): Array[LogisticRegressionModelsWithResults] = {
  ...
  private def irradiateGeneration(
    parents: Array[LogisticRegressionConfig],
    mutationCount: Int,
    ...
    var runSet = _initialGenerationMode match {
      ...
      case "permutations" =>
        val startingPool = new HyperParameterFullSearch()
          ...
          .initialGenerationSeedLogisticRegression(
            _logisticRegressionNumericBoundaries
          )
  ...
  def generateIdealParents(
    results: Array[LogisticRegressionModelsWithResults]
  ): Array[LogisticRegressionConfig] = {
  ...
  /**
    ...
    *
    * @param paramsToTest Array of LogisticRegression Configuration (hyper parameter settings) from the post-run model
    *                     inference
