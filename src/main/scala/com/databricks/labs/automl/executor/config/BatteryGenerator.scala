...
/**
  ...
  */
class BatteryGenerator(predictionType: String) extends BatteryDefaults with ConfigurationDefaults {


  private[config] val batteryType: PredictionType = BatteryGenerator.predictionTypeAssignment(predictionType)
  ...
  def setModelsToTest(modelFamilies: Array[String]): this.type = {
    ...
    modelFamilies foreach { x =>
      ...
      require(allowableModels.contains(definedModel), s"ModelsToTest setting $x is not allowable.  Must be one of:" +
  ...
  /**
    ...
    */
  def generateDefaultConfigs(labelCol: String, featuresCol: String, mlFlowTrackingURI: String,
                             mlFlowExperimentName: String, mlFlowAPIToken: String, modelSaveDirectory: String,
                             inferenceConfigSaveLocation: String,
                             mlFlowLoggingMode: String="full", mlFlowBestSuffix: String="_best",
                             mlFlowLoggingFlag: Boolean=true, mlFlowLogArtifactsFlag: Boolean=true,
                             fieldsToIgnoreInModeling: Array[String]=Array.empty[String]): Array[InstanceConfig] = {

    val defaultBuffer: ArrayBuffer[InstanceConfig] = ArrayBuffer[InstanceConfig]()

    _modelsToTest foreach { x =>

      val defaultGenericConfig: GenericConfig = new GenericConfigGenerator(predictionType)
        .setLabelCol(labelCol)
        .setFeaturesCol(featuresCol)
        .setFieldsToIgnoreInVector(fieldsToIgnoreInModeling)
        .getConfig

      val modelSpecificConfig = new ConfigurationGenerator(modelToStandardizedString(x), predictionType,
  ...
  /**
    ...
    */
  @deprecated("Main Config accessor will be replaced in future versions by InstanceConfig().")
  def generateDefaultMainConfigs(labelCol: String, featuresCol: String, mlFlowTrackingURI: String,
                                 mlFlowExperimentName: String, mlFlowAPIToken: String, modelSaveDirectory: String,
                                 inferenceConfigSaveLocation: String,
                                 mlFlowLoggingMode: String="full", mlFlowBestSuffix: String="_best",
                                 mlFlowLoggingFlag: Boolean=true, mlFlowLogArtifactsFlag: Boolean=true,
                                 fieldsToIgnoreInModeling: Array[String]=Array.empty[String]): Array[MainConfig] = {

    val mainConfigBuffer: ArrayBuffer[MainConfig] = ArrayBuffer[MainConfig]()

    val instanceConfigs = generateDefaultConfigs(labelCol, featuresCol, mlFlowTrackingURI, mlFlowExperimentName,
      mlFlowAPIToken, modelSaveDirectory, inferenceConfigSaveLocation, mlFlowLoggingMode, mlFlowBestSuffix,
      mlFlowLoggingFlag, mlFlowLogArtifactsFlag, fieldsToIgnoreInModeling)
