...
trait BatteryDefaults {
  ...
  def modelSelection(predictionType: PredictionType): Array[ModelSelector] = {
    ...
    predictionType match {
      case PredictionType.Regressor => RegressorModels.values.toArray.map(_.toString).map(x => ModelSelector.withName(x))
      case PredictionType.Classifier => ClassiferModels.values.toArray.map(_.toString).map(x => ModelSelector.withName(x))
      case _ => throw new UnsupportedOperationException(s"PrecitionType ${predictionType.toString} is not a supported" +
  ...
  def modelToStandardizedString(modelType: ModelSelector): String = {
    ...
    modelType match {
      ...
      case _ => throw new UnsupportedOperationException(s"'${modelType.toString}' is not supported.")
    }
