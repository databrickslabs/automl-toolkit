...
class ZipRegisterTempTransformerTest extends AbstractUnitSpec {
  ...
  ignore should "add Id column, retain feature columns and create temp view of original dataset" in {
    val testVars = PipelineTestUtils.getTestVars()
    val featureColumns =
      Array("age_trimmed", "workclass_trimmed", "fnlwgt_trimmed", "label")
...
//    LogManager.getRootLogger.setLevel(Level.DEBUG)
    ...
    assert(
      transformedAdultDf.count() == 99,
      "transformed table rows shouldn't have changed"
    )
    assert(
      transformedAdultDf.columns
        .contains(AutoMlPipelineMlFlowUtils.AUTOML_INTERNAL_ID_COL),
      "Id column should have been generated"
    )
  }
