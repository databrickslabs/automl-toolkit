...
class FeatureEngineeringOutputDfTest extends AbstractUnitSpec {
  ...
  "it" should "do this" in {
      ...
      .createDataFrame(
        (List.fill(50)(
          Row(
            ...
            scala.math.abs(scala.util.Random.nextLong()) + "",
            Date.valueOf(
              s"${startYear + random.nextInt((endYear - startYear) + 1)}-${startMonth + random
                .nextInt((endMonth - startMonth) + 1)}-${startDay + random
                .nextInt((endDay - startDay) + 1)}"
            ),
            Timestamp.valueOf(
              s"${startYear + random.nextInt((endYear - startYear) + 1)}-${startMonth + random
                .nextInt((endMonth - startMonth) + 1)}-${startDay + random
                .nextInt((endDay - startDay) + 1)} ${startMonth + random
                .nextInt((endMonth - startMonth) + 1)}:${startMonth + random
                .nextInt((endMonth - startMonth) + 1)}:${startMonth + random
                .nextInt((endMonth - startMonth) + 1)}.${startMonth + random
                .nextInt((endMonth - startMonth) + 1)}"
            ),
            "pass"
          )
        ) ++ List.fill(50)(
    ...
    val runner = FamilyRunner(sourceDF, Array(randomForestConfig))
