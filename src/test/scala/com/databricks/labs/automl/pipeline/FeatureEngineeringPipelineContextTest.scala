...
class FeatureEngineeringPipelineContextTest extends AbstractUnitSpec {
  ...
  ignore should "correctly generate feature engineered dataset" in {
    ...
    val randomForestConfig = ConfigurationGenerator.generateConfigFromMap(
      "RandomForest",
      "classifier",
      overrides
    )
    ...
    assert(
      pipelineModel
        ...
        .count() == 99,
      "Total row count shouldn't have changed"
    )
  }

  //
  ignore should "run train, save/load pipeline and predict" in {
    val testVars = PipelineTestUtils.getTestVars()
    val overrides = Map(
      "labelCol" -> "label",
      "mlFlowLoggingFlag" -> false,
    ...
    val runner =
      FamilyRunner(testVars.df, Array(randomForestConfig)).executeWithPipeline()
    val pipelineModel = runner.bestPipelineModel("RandomForest")
    val bcPipelineModel =
      testVars.df.sparkSession.sparkContext.broadcast(pipelineModel)

    val predictDf = bcPipelineModel.value.transform(testVars.df.drop("label"))
    assert(
      predictDf.count() == testVars.df.count(),
      "Inference df count should have matched the input dataset"
    )
    assert(
      testVars.df.columns
        .filterNot("label".equals(_))
        .forall(item => predictDf.columns.contains(item)),
    ...
    val pipelineSavePath = AutomationUnitTestsUtil
      .getProjectDir() + "/target/pipeline-tests/infer-final-pipeline"
    pipelineModel.write.overwrite().save(pipelineSavePath)
    PipelineModel
      .load(pipelineSavePath)
      .transform(testVars.df.drop("label"))
      .show(100)
  }
  ...
  it should "run train pipeline" in {
    ...
    val randomForestConfig = ConfigurationGenerator
      .generateConfigFromMap("RandomForest", "classifier", overrides)
    val runner = FamilyRunner(adultDfCleaned, Array(randomForestConfig))
      .executeWithPipeline()
    val predictDf = runner
